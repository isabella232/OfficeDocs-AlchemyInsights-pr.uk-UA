---
title: Змінити шрифт у Outlook.com
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "268"
- "1919"
- "8000013"
ms.assetid: 824035f6-90d3-4ea2-a92b-6b73b4ec0076
ms.openlocfilehash: 9b7106b78435e71d9e23662e7b5b71e8099e3d41
ms.sourcegitcommit: 631cbb5f03e5371f0995e976536d24e9d13746c3
ms.translationtype: MT
ms.contentlocale: uk-UA
ms.lasthandoff: 04/22/2020
ms.locfileid: "43760757"
---
# <a name="change-font-or-font-size"></a>Як змінити розмір шрифту або шрифту

Під час читання електронної пошти в Outlook.com можна використовувати функцію масштабування веб-переглядача, щоб збільшити або зменшити текст на екрані.
  
Щоб змінити шрифт або розмір шрифту під час створення повідомлення:
  
1. У нижній частині області створення, виберіть **Шрифт** <img src='data:image/png;base64,' /> або **розмір шрифту** <img src='data:image/png;base64,' /> Іконки.
2. Виберіть шрифт або розмір шрифту, який ви хочете використовувати.

Якщо меню параметри форматування не відображається в нижній частині області створення, виберіть **параметри форматування** <img src='data:image/png;base64,' /> Значок.
  
Щоб отримати додаткові відомості, див. [змінити розмір шрифту та зовнішній вигляд електронної пошти в Outlook.com](https://support.office.com/article/0b4eb323-23fc-4d5d-adbf-cae14c9c0386?wt.mc_id=Office_Outlook_com_Alchemy).