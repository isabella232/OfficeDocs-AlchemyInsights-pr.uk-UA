---
title: Outlook.com списки контактів
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.custom:
- "1850"
- "9000258"
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.openlocfilehash: c6cf17b7a6fc2c40e8a18de3a5be6e1c6969156b
ms.sourcegitcommit: 631cbb5f03e5371f0995e976536d24e9d13746c3
ms.translationtype: MT
ms.contentlocale: uk-UA
ms.lasthandoff: 04/22/2020
ms.locfileid: "43760541"
---
# <a name="organizing-your-outlookcom-mailbox"></a>Упорядкування поштової скриньки Outlook.com

Список контактів – це набір адрес електронної пошти та корисний для надсилання електронної пошти групі людей. Щоб створити список контактів:

1. Перейти до [людей](https://outlook.live.com/people/) <img src='data:image/png;base64,' />.
1. Угорі сторінки виберіть стрілку <img src='data:image/png;base64,' /> поруч із пунктом **створити контакт** і виберіть **новий список контактів**.
1. Введіть ім'я для списку, а потім додайте імена або адреси електронної пошти.
1. Виберіть **створити**.

Дізнайтеся [, як створювати, переглядати та редагувати контакти та списки контактів у Outlook.com](https://support.office.com/article/5b909158-036e-4820-92f7-2a27f57b9f01?wt.mc_id=Office_Outlook_com_Alchemy).