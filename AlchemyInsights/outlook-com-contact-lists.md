---
title: Списки контактів у Outlook.com
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ms.custom:
- "1850"
- "9000258"
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.openlocfilehash: 7e724b7b78e924a1f4d61193c900ade6b9a59c6c
ms.sourcegitcommit: c6692ce0fa1358ec3529e59ca0ecdfdea4cdc759
ms.translationtype: MT
ms.contentlocale: uk-UA
ms.lasthandoff: 09/14/2020
ms.locfileid: "47752236"
---
# <a name="organizing-your-outlookcom-mailbox"></a>Упорядкування поштової скриньки Outlook.com

Список контактів – це набір адрес електронної пошти та корисний для надсилання повідомлень електронної пошти групі користувачів. Щоб створити список контактів, виконайте наведені нижче дії.

1. Перехід до інших [користувачів](https://outlook.live.com/people/) <img src='data:image/png;base64,' />.
1. У верхній частині сторінки клацніть стрілку <img src='data:image/png;base64,' /> поруч із кнопкою **створити контакт** , а потім виберіть елемент **Створити список контактів**.
1. Введіть ім'я для списку, а потім додайте імена або адреси електронної пошти.
1. Натисніть кнопку **створити**.

Дізнайтеся більше про те, як [створювати, переглядати та редагувати контакти та списки контактів у Outlook.com](https://support.office.com/article/5b909158-036e-4820-92f7-2a27f57b9f01?wt.mc_id=Office_Outlook_com_Alchemy).