---
title: Outlook.com списки контактів
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ms.custom:
- "1850"
- "9000258"
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.openlocfilehash: a2bb861c4e8bc158b49138816553a37ff72ff8f0079cd6fdf98536697c857715
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: uk-UA
ms.lasthandoff: 08/05/2021
ms.locfileid: "54111961"
---
# <a name="organizing-your-outlookcom-mailbox"></a>Упорядкування поштової скриньки Outlook.com

Список контактів – це набір адрес електронної пошти, який зручно використовувати, щоб надсилати електронні листи групі користувачів. Щоб створити список контактів:

1. Перейти до списку ["Контакти"](https://outlook.live.com/people/) <img src='data:image/png;base64,' />.
1. Угорі сторінки натисніть стрілку <img src='data:image/png;base64,' /> поруч із **пунктом Створити контакт,** а **потім виберіть створити список контактів**.
1. Введіть ім'я списку, а потім додайте імена або адреси електронної пошти.
1. Натисніть **кнопку Створити**.

Дізнайтеся більше про створення, перегляд і редагування контактів і їх [списків у Outlook.com.](https://support.office.com/article/5b909158-036e-4820-92f7-2a27f57b9f01?wt.mc_id=Office_Outlook_com_Alchemy)