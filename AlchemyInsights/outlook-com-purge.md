---
title: Outlook.com.
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "1964"
- "9000336"
ms.openlocfilehash: 82ee2a56741655be3070b09eeb4e1e423b648250154af7448e230fe2ef1c1950
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: uk-UA
ms.lasthandoff: 08/05/2021
ms.locfileid: "54095779"
---
# <a name="permanently-delete-email-in-outlookcom"></a>Остаточне видалення електронної пошти на Outlook.com

Якщо потрібно видалити електронний лист або очистити папку "Вхідні":

1. Виберіть повідомлення, які потрібно видалити (або виділіть коло <img src='data:image/png;base64,' />
 поруч із **пунктом "Вхідні"** у верхній частині списку повідомлень, щоб вибрати всі повідомлення в папці "Вхідні").
1. У верхній частині сторінки виберіть Видалити **(для** всієї папки Очистити папку або Очистити виділені **інші,** якщо активовано функцію   /   "Активні вхідні").
1. Щоб видалити всі видалені електронні [](https://outlook.live.com/mail/deleteditems) листи, відкрийте папку "Видалені" й виберіть Очистити **папку,** а потім у верхній частині сторінки виберіть Відновлювані елементи, а потім **–** Очистити папку ще раз. 

Докладні відомості див. [в цій Outlook.com.](https://support.office.com/article/a9b63739-5392-412a-8e9a-d4b02708dee4?wt.mc_id=Office_Outlook_com_Alchemy)