---
title: 8000019. Додавання та імпорт контактів
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000019"
- "2002"
ms.assetid: ''
ms.openlocfilehash: 8e9535da046f21cc35ac9704492d8cd04e770aa997a6995f00dd3953241e471b
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: uk-UA
ms.lasthandoff: 08/05/2021
ms.locfileid: "53999801"
---
# <a name="import-contacts"></a>Імпорт контактів

Перш ніж імпортувати інтернет-версія Outlook до папки, знадобиться експортувати їх з іншого постачальника послуг електронної пошти .csv файлів. Потім виконайте наведені нижче вказівки.

1. У нижній частині сторінки виберіть пункт <img src='data:image/png;base64,' />
 **Люди**.
2. На панелі інструментів виберіть **Керувати**  >  **імпортом контактів**.
3. Натисніть **кнопку** Огляд, виберіть CSV-файл, а потім натисніть кнопку **Відкрити**.
4. Натисніть кнопку **Імпортувати**.

Докладні відомості див. [в Outlook.](https://support.office.com/article/bb796340-b58a-46c1-90c7-b549b8f3c5f8#ID0EAACAAA=Outlook_on_the_web)

